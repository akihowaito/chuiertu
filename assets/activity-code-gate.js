(function(){
  ...
  if(!document.getElementById(STYLE_ID)){
    ...
    style.textContent=`
      ...
      .code-gate__button{min-height:42px;padding:9px 18px;border:1.5px solid #f2a9ca;border-radius:12px;background:#fff;color:#df5b9d;font:900 13px/1.2 "Microsoft YaHei",sans-serif;cursor:pointer;box-shadow:0 4px 0 #f8d3e4;transition:transform .18s,background .18s}
      ...
      .code-gate__status{display:inline-flex;align-items:center;gap:6px;min-height:34px;padding:7px 12px;border-radius:999px;background:#fff7fb;color:#a38596;font:800 12px/1.2 "Microsoft YaHei",sans-serif;border:1px solid #f6d3e3}
      ...
      .code-gate-modal[hidden]{display:none}.code-gate-dialog{width:min(430px,100%);padding:26px;border:2px solid #f3a5c9;border-radius:18px;background:linear-gradient(135deg,#fff8fc,#f3fbff);box-shadow:0 12px 0 rgba(154,205,235,.22),0 28px 70px rgba(72,44,76,.24);font-family:"Microsoft YaHei",sans-serif}
      .code-gate-dialog h2{margin:0 0 8px;color:#644f66;font-size:22px}.code-gate-dialog p{margin:0 0 18px;color:#968294;font-size:13px;line-height:1.65}
      .code-gate-input{width:100%;height:50px;box-sizing:border-box;padding:0 14px;border:2px solid #d7eefa;border-radius:12px;outline:0;background:#fff;color:#5e4d60;font:900 16px/1 monospace;text-transform:uppercase;letter-spacing:1px}
      ...
      .code-gate-actions{display:grid;grid-template-columns:1fr 1.35fr;gap:10px}.code-gate-actions button{min-height:44px;border-radius:12px;font:900 13px/1 "Microsoft YaHei",sans-serif;cursor:pointer}
  ...
  async function mount(options){
    ...
    async function redeem(){
      ...
      try{
        ...
        state={code,digest:codeDigest,prizeId:record.prizeId,receiptId:record.receiptId,status:used.includes(codeDigest)?'completed':'active',activatedAt:new Date().toISOString()};
